...
_Noreturn void kernel_entry(Multiboot_info* boot_info) {
    ...
    if ((boot_flags & MULTIBOOT_INFO_HAS_VIDEO_INFO) != 0) {
        ...
        if (vbe_mode_info->phys_base_ptr != 0) {
            ...
            int const size = vbe_mode_info->x_resolution * vbe_mode_info->y_resolution * byte_per_pixel;

            char* const vram = (char*)vbe_mode_info->phys_base_ptr;

            int const rs = vbe_mode_info->red_mask_size;
            int const gs = vbe_mode_info->green_mask_size;
            int const bs = vbe_mode_info->blue_mask_size;

            int const rp = vbe_mode_info->red_field_position;
            int const gp = vbe_mode_info->green_field_position;
            int const bp = vbe_mode_info->blue_field_position;

            /* uint32_t rgb_to_direct_color(uint8_t r, uint8_t g, uint8_t b) { */
            /* for (int i = 0; i < count; i++) { */
            /* vram[]; */
            /* } */
            /* } */

            for (int i = 0; i < size; i += byte_per_pixel) {
                /* if ((rs + gs + bs) == vbe_mode_info->bits_per_pixel) { */
                if ((rs + gs + bs + vbe_mode_info->rsvd_mask_size) == vbe_mode_info->bits_per_pixel) {
                    vram[i] = (0 << rp) + (0 << gp) + (31 << bp);
                } else {
                    vram[i] = (31 << rp) + (63 << gp) + (31 << gp);
                }

                /*
                 * if (6 == vbe_mode_info->memory_model_type) {
                 *     [> Direct Color <]
                 *     vram[i] = (i % 2) ? 0x55 : 0xAA;
                 * } else {
                 *     vram[i] = 0xff;
                 * }
                 */
