...
pub use self::address::{Alignment, PhysicalAddress, VirtualAddress};
...
pub fn clean_bss_section() {
    let begin = kernel_bss_section_addr_begin().into();
...
pub fn init<U: Into<Region>, T: Iterator<Item = U>>(regions: &region::Adapter<Item = U, Target = T>, remap_requests: &[IdenticalReMapRequest]) -> Result<(), Error> {
    ...
    let mut usable_memory_regions = regions.iter().filter(|region| {
        let a: usize = kernel_addr_begin_physical.into();
        a<= region.base_addr().into()
    });
    ...
    let fuck: usize = free_memory_region.base_addr().into();
    let free_region_addr_end: usize = fuck + free_memory_region.size();
    let mut allocator = EarlyAllocator::new(free_region_addr_begin.into(), free_region_addr_end);

    let (frames, count_frames) = allocate_frames(&mut allocator);

    // Add the size of `frames` to the kernel size.
    let base_addr = PhysicalAddress::new(allocator.to_addr_begin());
    update_kernel_addr_end_physical(base_addr);

    let fuck: usize = base_addr.into();
    println!("free region: 0x{:x} - 0x{:x}, {}KB", base_addr, fuck + count_frames * frame::SIZE, count_frames * frame::SIZE / 1024);

    // Initialize frames.
    unsafe {
        let base: usize = base_addr.into() / frame::SIZE;
        for (i, f) in core::slice::from_raw_parts_mut(frames.as_ptr(), count_frames).into_iter().enumerate() {
            f.set_number(base + i)
        }
    };

    let bman = BuddyAllocator::new(base_addr.into(), frames, count_frames, FrameAdapter::new());
